...
const messages = {
	...
	portugues: {
			...
			description: 'digite cada palavra no ritimo',
			play: 'JOGAR'
	},
};
...
//VAR'S
	...
	var{ 
		...
		ctx,
		GAME_AUTOPLAY,
		GAME_PERFECT_OFFSET_SECONDS,
		GAME_OK_OFFSET_SECONDS,
		GAME_WORDS_PER_LEVEL,
		GAME_RESTS_PER_LEVEL,
		GAME_STARTING_BPM,
		GAME_BPM_CHANGE_RATE,
		GAME_MAX_FAILS,
		GFX_BACKGROUND_COLOR,
		GFX_BACKGROUND_BEAT_COLOR,
		GFX_BACKGROUND_GRID_COLOR,
		GFX_BACKGROUND_GRID_ANGLE,
		GFX_BACKGROUND_GRID_SPACING,
		GFX_BACKGROUND_GRID_STROKE,
		GFX_BACKGROUND_GRID_SCROLL_X,
		GFX_BACKGROUND_GRID_SCROLL_Y,
		GFX_BACKGROUND_GRID_BUMP,
		GFX_WORD_SCALE,
		GFX_WORD_OFFSET_X,
		GFX_WORD_BASE_COLOR,
		GFX_WORD_OVER_COLOR,
		GFX_WORD_UNDERLINE_STROKE,
		GFX_WORD_UNDERLINE_DASH,
		GFX_WORD_BOX_STROKE,
		GFX_WORD_BOX_SCALE,
		GFX_WORD_BOX_OFFSET_X,
		GFX_WORD_BOX_OFFSET_Y,
		GFX_WORD_BOX_SHIFT_FACTOR,
		GFX_WORD_BOX_SYMBOL_SCALE,
		GFX_WORD_BOX_SYMBOL_STROKE,
		GFX_WORD_BOX_CHECK_COLOR,
		GFX_WORD_BOX_X_COLOR,
		GFX_WORD_FLIP_OFFSET_CHANCE,
		GFX_WORDLINE_OFFSET_Y,
		GFX_WORDLINE_PUSHDOWN_FRAMES,
		GFX_WORDLINE_FADEOUT_RATE,
		GFX_WORDLINE_LETTER_TWIST_MULTIPLIER,
		GFX_UI_FONT_SIZE,
		GFX_UI_PADDING,
		GFX_UI_FAIL_BOX_SIZE,
		GFX_UI_FAIL_BOX_STROKE,
		GFX_UI_FAIL_BOX_SPACING,
		GFX_UI_TIME_SIGNATURE_SCALING
	} = CanvasGame();
...
//FUNCTIONS
	...
	// Mensagem do menu inicial , usado na setup();
	function updateIdioma(newIdioma) {
		idioma = newIdioma;
		setup();
	}
	function updateLifes(newLife) {
		Live = newLife;
		setup();
	}
	function updateTotorial(){
	...
	function CanvasGame(){
		...
		const GAME_PERFECT_OFFSET_SECONDS = .75;
	...
	function setup() {
		...
		words.push(new Word(messages[idioma].play));
		words.push(new Tutorial());
	}
	function loop() {
...
// Classe de padrão musical
		...
		constructor(letters) {
			if (isNaN(letters)) {
				...
				while ((this.value == null || usedWords.has(this.value)) && tries > 0) {
					this.value = dictionary[idioma][letters][Math.randInt(0, dictionary[idioma][letters].length - 1)].toUpperCase();
					tries--;
				}
	...
	class Tutorial {
		constructor(measuresLeft) {
			...
			this.ctx.fillText(messages[idioma].title, GFX_WORD_BOX_OFFSET_X, this.canvas.height * .25);
			this.ctx.font = (this.canvas.height * GFX_WORD_SCALE) / 8 + 'px Cambo';
			this.ctx.fillText(messages[idioma].title, GFX_WORD_BOX_OFFSET_X, this.canvas.height * .45);
			this.ctx.font = (this.canvas.height * GFX_WORD_SCALE) / 4 + 'px Cambo';
			this.ctx.fillText(messages[idioma].title, GFX_WORD_BOX_OFFSET_X, this.canvas.height * .66);
		}
		keystroke() { }
		finalize() { }
	}
